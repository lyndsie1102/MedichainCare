...
def get_doctor_details(
    ...
    return DoctorOut(
        id=current_user.id, name=current_user.name, specialty=doctor.specialty
    )
...
def get_doctors(
    ...
    doctors = (
        ...
        .filter(User.role == RoleEnum.DOCTOR, User.id != current_user.id)
        .all()
    )
...
def get_doctor_dashboard(
    ...
    research_symptoms = (
        db.query(Symptom)
        .join(Consent)
        .filter(Consent.consent_type == ConsentPurpose.RESEARCH, Consent.is_granted)
        .all()
    )
    ...
    for symptom in sorted_symptoms:
        ...
        if search:
            ...
            if search_text not in symptom.symptoms.lower() and (
                user.name is None or search_text not in user.name.lower()
            ):
                continue
...
@router.get(
    "/symptom_details/{symptom_id}",
    response_model=Union[PatientSymptomDetails, SymptomDetails],
)
def get_symptom_details(
    ...
    if isinstance(images, str):
        ...
        if images.strip() == "[]":
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="Invalid format for image paths"
                )
    ...
    referral = (
        db.query(Referral)
        .filter(
            Referral.symptom_id == symptom.id, Referral.referral_doctor_id == doctor.id
        )
        .first()
    )
    ...
    if is_gp:
        ...
        raise HTTPException(
            status_code=403, detail="You are not authorized to view this symptom."
        )
...
def _get_test_type(db: Session, symptom_id: int) -> str:
    ...
    if request:
        test_type = (
            db.query(TestType).filter(TestType.id == request.test_type_id).first()
        )
...
def _get_test_results(db: Session, symptom_id: int) -> List[TestResultOut]:
    ...
    if request:
        for result in request.test_results:
            ...
            results.append(
                TestResultOut(
                    uploadedAt=result.uploaded_at, files=files, summary=result.summary
                )
            )
    return results
...
def refer_symptom(
    ...
    referral_doc = (
        db.query(Doctor).filter(Doctor.id == referral_data.referral_doctor_id).first()
    )
    if not referral_doc:
    ...
    if not symptom.consent_referral:
        raise HTTPException(
            status_code=403, detail="Referral consent not granted by patient."
        )
...
def assign_lab(
    ...
    if not symptom or not lab or not test_type:
        raise HTTPException(
            status_code=404, detail="Symptom or Lab or Test type not found"
        )
