...
async def schedule_appointment(test_request_id: int, 
    ...
    appointment = db.query(Appointment).filter(Appointment.test_request_id == test_request_id).first()
    if appointment and appointment.status == AppointmentStatus.CANCELLED:
        ...
        return {"message": f"Appointment rescheduled successfully for {scheduled_time_str}"}
