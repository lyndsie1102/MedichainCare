...
def get_symptom_details(
    ...
    for diagnosis in diagnoses:
        ...
        if doctor:
            doctor_name = (
                doctor.user.name
            )  # Access the User object associated with the Doctor
        else:
    ...
    test_requests = (
        db.query(TestRequest).filter(TestRequest.symptom_id == symptom_id).all()
    )
    ...
    if test_requests:
        ...
        appointment = (
            ...
            .filter_by(test_request_id=latest_test_request.id)
            .first()
        )
        ...
        if appointment and appointment.scheduled_at:
            ...
            lab_staff = (
                db.query(LabStaff).filter_by(id=appointment.lab_staff_id).first()
            )
            if lab_staff:
    ...
    for test_request in test_requests:
        ...
        test_type = (
            db.query(TestType).filter(TestType.id == test_request.test_type_id).first()
        )
        if test_type:
            test_type_name = (
                test_type.name
            )  # Assuming we only need to store one test type name for the symptom

        test_results = (
            db.query(TestResults)
            .filter(TestResults.test_request_id == test_request.id)
            .all()
        )
        ...
        for test_result in test_results:
            ...
            for file in test_result.files:
                files_info.append(
                    {"name": file.get("name", "Unknown"), "url": file.get("url", "")}
                )
...
def get_symptom_history(
    ...
    if start_date and end_date:
        ...
        if start_date_obj > end_date_obj:
            raise HTTPException(
                status_code=400, detail="Start date cannot be later than the end date."
            )
    ...
    for s in symptoms:
        ...
        consent_types = [
            consent.consent_type for consent in consents if consent.is_granted
        ]
