...
class Util(object):
    ...
    def random_slice(n, step=None):
        return slice(
            randint(-n - 2, n + 2), randint(-n - 2, n + 2), step or randint(-5, 5) or 1
        )
    ...
    def test_set_default_endian(self):
        ...
        for default_endian in "big", "little":
            ...
            for endian in "big", "little", None:
                ...
                self.assertEqual(a.endian, default_endian if endian is None else endian)
...
class GetMaskedIndexTests(unittest.TestCase, Util):
    ...
    def test_random(self):
        for a in self.randombitarrays():
            ...
            mask = urandom_2(n)
            self.assertEqual(list(a[mask]), [a[i] for i in range(n) if mask[i]])
...
class DelMaskedIndexTests(unittest.TestCase, Util):
    ...
    def test_random_mask(self):
        for a in self.randombitarrays():
            ...
            mask = urandom_2(n)
            del b[mask]
            self.assertEqual(b, bitarray(a[i] for i in range(n) if not mask[i]))
    ...
    def test_imported(self):
        ...
        b = bitarray(buffer=a)
        self.assertFalse(b.readonly)
        self.assertRaises(BufferError, b.__delitem__, bitarray("00001111 00110011"))
        # even though we don't delete anything, raise error
...
class ExtendTests(unittest.TestCase, Util):
    ...
    def test_sequence(self):
        lst = [0, 1, 0, 1, 1]
        for x in [lst, tuple(lst), bytes(lst), bytearray(lst), array.array("b", lst)]:
            self.assertEqual(len(x), 5)  # sequences have len, iterables not
            a = bitarray()
            a.extend(x)
...
class CountTests(unittest.TestCase, Util):
    ...
    def test_random_slice(self):
        for n in range(500):
            ...
            v = randrange(2)
            s = self.random_slice(n)
            self.assertEqual(a.count(v, s.start, s.stop, s.step), a[s].count(v))
...
class IndexTests(unittest.TestCase, Util):
    ...
    def test_empty(self):
        ...
        for _ in range(50):
            ...
            right = getrandbits(1)
            self.assertEqual(
                z.find(empty, right=right), self.find_empty(n, right=right)
            )

            start = randint(-5, 5)
            self.assertEqual(
                z.find(empty, start, right=right),
                self.find_empty(n, start, right=right),
            )

            stop = randint(-5, 5)
            self.assertEqual(
                z.find(empty, start, stop, right),
...
class FileTests(unittest.TestCase, Util):
    ...
    def test_fromfile_erros(self):
        ...
        with open(self.tmpfname, "wb") as fi:
            self.assertRaisesMessage(UnsupportedOperation, "read", a.fromfile, fi)

        with open(self.tmpfname, "r") as fi:
            self.assertRaisesMessage(
                TypeError,
                ".read() did not return " "'bytes', got 'str'",
                a.fromfile,
                fi,
            )
    ...
    def test_frombytes_invalid_reader(self):
        ...
        f = Reader()
        self.assertRaisesMessage(
            TypeError, ".read() did not return " "'bytes', got 'int'", a.fromfile, f
        )
