...
from bitarray.test_bitarray import (
    Util,
    skipIf,
    is_pypy,
    urandom_2,
...
class Random_P_Tests(unittest.TestCase):
    ...
    def test_seed(self):
        ...
        self.assertEqual(random_p(32), bitarray("10011101111111101001011011101111"))
        # test small p
        a = random_p(5000, 0.001)
        self.assertEqual(list(a.search(1)), [286, 687, 806, 2905])
        # general case
        self.assertEqual(
            random_p(100, 0.7)[:32], bitarray("10101111001010110010111111000111")
        )
...
class CountN_Tests(unittest.TestCase, Util):
    ...
    def test_empty(self):
        ...
        self.assertRaisesMessage(
            ValueError, "n = 1 larger than bitarray " "length 0", count_n, a, 1
        )
    ...
    def test_simple(self):
        ...
        # n > len(a)
        self.assertRaisesMessage(
            ValueError, "n = 49 larger than bitarray " "length 48", count_n, a, 49
        )
        # n > a.count(0)
        self.assertRaisesMessage(
            ValueError,
...
class CorrespondAllTests(unittest.TestCase, Util):
    ...
    def test_basic(self):
        ...
        self.assertRaises(
            ValueError, correspond_all, bitarray("01", "little"), bitarray("11", "big")
        )
...
class IntervalsTests(unittest.TestCase, Util):
    ...
    def test_explicit(self):
        for s, lst in [
            ...
            ("0", [(0, 0, 1)]),
            ("1", [(1, 0, 1)]),
            ("00111100 0000011", [(0, 0, 2), (1, 2, 6), (0, 6, 13), (1, 13, 15)]),
        ]:
            a = bitarray(s)
...
class SC_Tests(unittest.TestCase, Util):
    ...
    def test_decode_header_errors(self):
        ...
        for c in 0x20, 0x21, 0x40, 0x80, 0xC0, 0xF0, 0xFF:
            self.assertRaisesMessage(
                ValueError, "invalid header: 0x%02x" % c, sc_decode, [c]
            )
        # invalid block head
        for c in 0xC0, 0xC1, 0xC5, 0xFF:
            self.assertRaisesMessage(
                ValueError, "invalid block head: 0x%02x" % c, sc_decode, [0x01, 0x10, c]
            )
    ...
    def test_decode_errors(self):
        ...
        msg = {
            4: "read 4 bytes got negative value: -2147483648",
            8: "decode error (n=4): 2147483648 >= 16",
        }
        self.assertRaisesMessage(
            ValueError, msg[PTRSIZE], sc_decode, b"\x01\x10\xc4\x01\x00\x00\x00\x80\0"
        )

        msg = {
            4: "read 4 bytes got negative value: -1",
            8: "decode error (n=4): 4294967295 >= 16",
        }
        self.assertRaisesMessage(
            ValueError, msg[PTRSIZE], sc_decode, b"\x01\x10\xc4\x01\xff\xff\xff\xff\0"
        )
    ...
    def test_block_type0(self):
        for k in range(0x01, 0xA0):
            ...
            a = ones(nbits, "little")
            b = bytearray(
                [0x01, nbits] if nbits < 256 else [0x02, nbits % 256, nbits // 256]
            )
            b.append(k)
            b.extend(a.tobytes())
...
class VLFTests(unittest.TestCase, Util):
    ...
    def test_decode_errors(self):
        ...
        for s in [
            ...
            b"\x50",
            b"\x60",
            b"\x70",  # no second byte, but padding > 4
        ]:
            self.assertRaisesMessage(
                ValueError, "invalid head byte: 0x%02x" % s[0], vl_decode, s
            )
