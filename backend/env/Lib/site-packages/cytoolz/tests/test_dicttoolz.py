...
class TestDict(object):
    ...
    def test_itemfilter(self):
        D, kw = self.D, self.kw
        assert itemfilter(lambda item: iseven(item[0]), D({1: 2, 2: 3}), **kw) == D(
            {2: 3}
        )
        assert itemfilter(lambda item: iseven(item[1]), D({1: 2, 2: 3}), **kw) == D(
            {1: 2}
        )
    ...
    def test_dissoc(self):
        ...
        assert dissoc(D({"a": 1}), "a", **kw) == dissoc(
            dissoc(D({"a": 1}), "a", **kw), "a", **kw
        )
    ...
    def test_assoc_in(self):
        ...
        assert assoc_in(D({"a": D({"b": 1})}), ["a", "b"], 2, **kw) == D(
            {"a": D({"b": 2})}
        )
    ...
    def test_update_in(self):
        ...
        assert update_in(D({"foo": "bar", 1: 50}), ["d", 1, 0], str, 20, **kw) == D(
            {"foo": "bar", 1: 50, "d": D({1: D({0: "20"})})}
        )
        # Verify immutability:
        d = D({"x": 1})
        oldd = d
    ...
    def test_factory(self):
        ...
        assert merge(
            defaultdict(int, D({1: 2})), D({2: 3}), factory=lambda: defaultdict(int)
...
class defaultdict(_defaultdict):
    def __eq__(self, other):
        return (
            super(defaultdict, self).__eq__(other)
            and isinstance(other, _defaultdict)
