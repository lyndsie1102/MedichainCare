...
def test_docstrings_uptodate():
    ...
    for key, (toolz_func, cytoolz_func) in d.items():
        ...
        toolz_doc = "\n".join(
            (line.strip() for line in convertdoc(toolz_func).splitlines())
        )
        cytoolz_doc = "\n".join(
            (line.strip() for line in cytoolz_func.__doc__.splitlines())
        )
        if toolz_doc not in cytoolz_doc:
            diff = list(
                differ.compare(toolz_doc.splitlines(), cytoolz_doc.splitlines())
            )
            fulldiff = list(diff)
