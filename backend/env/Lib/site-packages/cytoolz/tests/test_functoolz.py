...
def test_memoize_key_signature():
    ...
    mf = memoize(
        lambda x, y=0: False,
        cache={((1,), frozenset((("y", 2),))): 2, ((1, 2), None): 3},
    )
    assert mf(1, y=2) == 2
    assert mf(1, 2) == 3
    ...
    mf = memoize(
        lambda x=0: False, cache={(None, frozenset((("x", 1),))): 1, ((1,), None): 2}
    )
    assert mf() is False
    assert mf(x=1) == 1
    assert mf(1) == 2
...
def generate_compose_test_cases():
    ...
    return (
        (
            ...
            (0,),
            {},  # positional and keyword args to the Composed object
            0,  # expected result
        ),
        ((inc,), (0,), {}, 1),
        ((double, inc), (0,), {}, 2),
        ((str, iseven, inc, double), (3,), {}, "False"),
        ((str, add), (1, 2), {}, "3"),
        ((str, inc, add_then_multiply), (1, 2), {"c": 3}, "10"),
    )
...
def generate_compose_left_test_cases():
    ...
    return tuple(
        ...
        for (compose_args, args, kwargs, expected) in generate_compose_test_cases()
    )
