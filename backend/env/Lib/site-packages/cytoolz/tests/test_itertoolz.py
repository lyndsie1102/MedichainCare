...
def test_merge_sorted():
    ...
    assert list(merge_sorted([5, 3, 1], [6, 4, 3], [], key=lambda x: -x)) == [
        6,
        5,
        4,
        3,
    ...
    assert list(merge_sorted([2, 1, 3], [1, 2, 3], key=lambda x: x // 3)) == [
        2,
        1,
        1,
        2,
    ...
    assert (
        "".join(merge_sorted("cba", "cba", "cba", key=lambda x: -ord(x))) == "cccbbbaaa"
    )
...
def test_mapcat():
    ...
    assert list(mapcat(reversed, [[3, 2, 1, 0], [6, 5, 4], [9, 8, 7]])) == list(
        range(10)
    )
...
def test_frequencies():
    assert frequencies(["cat", "pig", "cat", "eel", "pig", "dog", "dog", "dog"]) == {
        "cat": 2,
        "eel": 1,
        "pig": 2,
        "dog": 3,
...
def test_reduceby():
    ...
    assert reduceby(
        lambda x: x["state"], lambda acc, x: acc + x["cost"], projects, 0
...
def test_key_as_getter():
    ...
    assert set(join(0, squares, 0, pows)) == set(
        join(lambda x: x[0], squares, lambda x: x[0], pows)
    )

    get = lambda x: (x[0], x[1])
    assert set(join([0, 1], squares, [0, 1], pows)) == set(
        join(get, squares, get, pows)
    )
...
def test_outer_join():
    result = set(
        join(identity, [1, 2], identity, [2, 3], left_default=None, right_default=None)
    )
...
def test_topk():
    ...
    assert topk(
        2,
        [{"a": 1, "b": 10}, {"a": 2, "b": 9}, {"a": 10, "b": 1}, {"a": 9, "b": 2}],
        key="a",
    ) == ({"a": 10, "b": 1}, {"a": 9, "b": 2})
