...
def expression(callable, rule_name, grammar):
    ...
    if num_args == 2:
        ...
        raise RuntimeError(
            "Custom rule functions must take either 2 or 5 "
    ...
    class AdHocExpression(Expression):
        def _uncached_match(self, text, pos, cache, error):
            result = (
                callable(text, pos)
                if is_simple
                else callable(text, pos, cache, error, grammar)
            )
...
class Expression(StrAndRepr):
    ...
    def _eq_check_cycles(self, other, checked):
        ...
        return (
            ...
            and self.identity_tuple == other.identity_tuple
        )
    ...
    def match_core(self, text, pos, cache, error):
        ...
        if (
            node is None
            and pos >= error.pos
            and (self.name or getattr(error.expr, "name", None) is None)
        ):
    ...
    def __str__(self):
        return "<%s %s>" % (self.__class__.__name__, self.as_rule())
...
class Regex(Expression):
    ...
    def _regex_flags_from_bits(self, bits):
        ...
        flags = "ilmsuxa"
        return "".join(
            flags[i - 1] if (1 << i) & bits else "" for i in range(1, len(flags) + 1)
        )

    def _as_rhs(self):
        return "~{!r}{}".format(
            self.re.pattern, self._regex_flags_from_bits(self.re.flags)
        )
...
class Compound(Expression):
    ...
    def _eq_check_cycles(self, other, checked):
        return (
            super()._eq_check_cycles(other, checked)
            and len(self.members) == len(other.members)
            and all(
                m._eq_check_cycles(mo, checked)
                for m, mo in zip(self.members, other.members)
                if (id(m), id(mo)) not in checked
            )
        )
...
class Lookahead(Compound):
    ...
    def _uncached_match(self, text, pos, cache, error):
        ...
        if (
            node is None
        ) == self.negativity:  # negative lookahead == match only if not found
    ...
    def _eq_check_cycles(self, other, checked):
        return (
            super()._eq_check_cycles(other, checked)
            and self.negativity == other.negativity
        )
