...
class Node(object):
    ...
    __slots__ = [
        "expr",  # The expression that generated me
        ...
        "children",
    ]  # List of child parse tree nodes
            ...
            % (
                ...
                (
                    (
                        ", children=[%s]"
                        % ", ".join(
                            [c.__repr__(top_level=False) for c in self.children]
                        )
                    )
                    if self.children
                    else ""
...
class RuleDecoratorMeta(type):
    def __new__(metaclass, name, bases, namespace):
        ...
        methods = [
            v for k, v in namespace.items() if hasattr(v, "_rule") and isfunction(v)
        ]
        if methods:
            ...
            methods.sort(
                key=(
                    (lambda x: x.func_code.co_firstlineno)
                    if version_info[0] < 3
                    else (lambda x: x.__code__.co_firstlineno)
                )
            )
            ...
            namespace["grammar"] = Grammar(
                "\n".join(
                    "{name} = {expr}".format(name=unvisit(m.__name__), expr=m._rule)
                    for m in methods
                )
            )
        return super(RuleDecoratorMeta, metaclass).__new__(
