...
class LengthTests(TestCase):
    ...
    def test_sequence(self):
        self.len_eq(
            Sequence(Regex("hi*"), Literal("lo"), Regex(".ingo")).match(
                "hiiiilobingo1234"
            ),
            12,
        )  # succeed
    ...
    def test_one_of(self):
        self.len_eq(
            OneOf(Literal("aaa"), Literal("bb")).match("aaa"), 3
        )  # first alternative
        self.len_eq(OneOf(Literal("aaa"), Literal("bb")).match("bbaaa"), 2)  # second
        self.assertRaises(
            ParseError, OneOf(Literal("aaa"), Literal("bb")).match, "aa"
        )  # no match
    ...
    def test_optional(self):
        self.len_eq(
            Sequence(Optional(Literal("a")), Literal("b")).match("b"), 1
        )  # contained expr fails
        self.len_eq(
            Sequence(Optional(Literal("a")), Literal("b")).match("ab"), 2
        )  # contained expr succeeds
    ...
    def test_one_or_more(self):
        ...
        self.len_eq(
            OneOrMore(Literal("b"), min=3).match("bbb"), 3
        )  # with custom min; success
        self.len_eq(
            Quantifier(Literal("b"), min=3, max=5).match("bbbb"), 4
        )  # with custom min and max; success
        self.len_eq(
            Quantifier(Literal("b"), min=3, max=5).match("bbbbbb"), 5
        )  # with custom min and max; success
        self.assertRaises(
            ParseError, OneOrMore(Literal("b"), min=3).match, "bb"
        )  # with custom min; failure
        self.assertRaises(
            ParseError, Quantifier(Literal("b"), min=3, max=5).match, "bb"
        )  # with custom min and max; failure
...
class TreeTests(TestCase):
    ...
    def test_sequence_nodes(self):
        ...
        s = Sequence(
            Literal("heigh", name="greeting1"),
            Literal("ho", name="greeting2"),
            name="dwarf",
        )
        text = "heighho"
        self.assertEqual(
    ...
    def test_one_of(self):
        ...
        self.assertEqual(
            o.match(text),
            Node(o, text, 0, 1, children=[Node(o.members[0], text, 0, 1)]),
        )
    ...
    def test_optional(self):
        ...
        self.assertEqual(
            expr.match(text),
            Node(expr, text, 0, 1, children=[Node(expr.members[0], text, 0, 1)]),
        )
...
class ErrorReportingTests(TestCase):
    ...
    def test_line_and_column(self):
        ...
        try:
            ...
            self.assertTrue(
                str(error).endswith(r"""didn't match at 'GOO' (line 2, column 4).""")
            )
...
class RepresentationTests(TestCase):
    ...
    def test_unicode_keep_parens(self):
        ...
        # ZeroOrMore
        self.assertEqual(
            str(Grammar('foo = "bar" ("baz" "eggs")* "spam"')),
            "foo = 'bar' ('baz' 'eggs')* 'spam'",
        )

        # Quantifiers
        self.assertEqual(
            str(Grammar('foo = "bar" ("baz" "eggs"){2,4} "spam"')),
            "foo = 'bar' ('baz' 'eggs'){2,4} 'spam'",
        )
        self.assertEqual(
            str(Grammar('foo = "bar" ("baz" "eggs"){2,} "spam"')),
            "foo = 'bar' ('baz' 'eggs'){2,} 'spam'",
        )
        self.assertEqual(
            str(Grammar('foo = "bar" ("baz" "eggs"){1,} "spam"')),
            "foo = 'bar' ('baz' 'eggs')+ 'spam'",
        )
        self.assertEqual(
            str(Grammar('foo = "bar" ("baz" "eggs"){,4} "spam"')),
            "foo = 'bar' ('baz' 'eggs'){,4} 'spam'",
        )
        self.assertEqual(
            str(Grammar('foo = "bar" ("baz" "eggs"){0,1} "spam"')),
            "foo = 'bar' ('baz' 'eggs')? 'spam'",
        )
        self.assertEqual(
            str(Grammar('foo = "bar" ("baz" "eggs"){0,} "spam"')),
            "foo = 'bar' ('baz' 'eggs')* 'spam'",
        )
        ...
        # Multiple sequences
        self.assertEqual(
            str(Grammar('foo = ("bar" "baz") / ("baff" "bam")')),
