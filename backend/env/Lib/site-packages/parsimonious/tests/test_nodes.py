...
class SimpleTests(TestCase):
    ...
    def test_visitation_exception(self):
        self.assertRaises(
            VisitationError, ExplosiveFormatter().visit, Node(Literal(""), "", 0, 0)
        )
    ...
    def test_repr(self):
        ...
        n = Node(
            ...
            0,
            3,
            children=[Node(Literal(" "), s, 3, 4), Node(Literal("ö"), s, 4, 5)],
        )
        self.assertEqual(
            repr(n),
            str(
                """s = {hai_o}\nNode({boogie}, s, 0, 3, children=[Node({space}, s, 3, 4), Node({o}, s, 4, 5)])"""
            ).format(
                hai_o=repr(s),
...
class SpecialCasesTests(TestCase):
    ...
    def test_generic_visit_NotImplementedError_unnamed_node(self):
        ...
        with self.assertRaises(NotImplementedError) as e:
            MyVisitor().parse("bar")
        self.assertIn(
            "No visitor method was defined for this expression: 'b'", str(e.exception)
        )
    ...
    def test_generic_visit_NotImplementedError_named_node(self):
        ...
        with self.assertRaises(NotImplementedError) as e:
            MyVisitor().parse("bar")
        self.assertIn(
            "No visitor method was defined for this expression: myrule = ~'[bar]'",
            str(e.exception),
        )
