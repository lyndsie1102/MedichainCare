...
from models import (Doctor, Patient, LabStaff, MedicalLab, TestRequest, TestResults, 
                    TestType, Diagnosis, Appointment, Symptom, GenderEnum, SymptomStatus,
                    ConsentPurpose, Consent, Referral, AppointmentStatus, TestRequestStatus)
...
def gp_doctor(test_user, test_db):
    ...
    doctor_user = test_user(
        ...
        username_suffix="gp_house",
        password="gp_doctor_user"
    )
...
def patient_user_with_gp(test_user, gp_doctor, test_db):
    ...
    patient_user = test_user(
        ...
        username_suffix="johndoe",
        password="testpassword123"
    )
    # ... (rest of the fixture is fine)
    patient_user.name = "John Doe"
    patient_record = Patient(user=patient_user, gp_id=gp_doctor.id, location="Testville", email="john.doe@test.com")
...
def lab_staff_user(test_user, medical_lab, test_db):
    ...
    staff_record = LabStaff(id=staff_user.id, location="Lab City", specialties="Bloodwork", lab_id=medical_lab.id)
...
def medical_lab(test_db):
    lab = MedicalLab(name="TestLab Inc.", location="Lab City", specialties="Bloodwork,Genetics")
    test_db.add(lab)
    test_db.commit()
    test_db.refresh(lab)
    return lab
...
def referred_symptom(test_db, symptom_for_gp, specialist_doctor):
    ...
    referral = Referral(
        ...
        referral_doctor_id=specialist_doctor.id
    )
...
def scheduled_appointment(test_db, assigned_test_request, lab_staff_user):
    ...
    scheduled_time = datetime.utcnow().replace(hour=10, minute=0, second=0, microsecond=0) + timedelta(days=5)
