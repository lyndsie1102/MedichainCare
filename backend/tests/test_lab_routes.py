...
from models import User, TestRequest, Appointment, SymptomStatus, TestResults, AppointmentStatus, TestRequestStatus
from datetime import datetime, timedelta
import io

def test_get_own_lab_staff_details(client: TestClient, lab_staff_user: User, lab_auth_token: str):
    """Test retrieving the current lab staff's own details."""
    response = client.get("/labs/lab-staff/me", headers={"Authorization": f"Bearer {lab_auth_token}"})
    
    assert response.status_code == 200
    data = response.json()
    assert data["name"] == lab_staff_user.name

def test_get_lab_dashboard(client: TestClient, lab_auth_token: str, assigned_test_request: TestRequest):
    """Test the lab dashboard can retrieve assigned test requests for the lab staff's lab."""
    response = client.get("/labs/test-requests", headers={"Authorization": f"Bearer {lab_auth_token}"})
...
def test_lab_dashboard_filter_by_status(client: TestClient, lab_auth_token: str, assigned_test_request: TestRequest):
    """Test filtering the lab dashboard by request status."""
    # Filter for PENDING, which should exist
    response = client.get("/labs/test-requests?status=pending", headers={"Authorization": f"Bearer {lab_auth_token}"})
    assert response.status_code == 200
    data = response.json()
    assert len(data) >= 1
    ...
    response = client.get("/labs/test-requests?status=uploaded", headers={"Authorization": f"Bearer {lab_auth_token}"})
    assert response.status_code == 200
    assert len(response.json()) == 0

def test_upload_lab_result(client: TestClient, test_db: Session, lab_auth_token: str, assigned_test_request: TestRequest):
    ...
    result_in_db = test_db.query(TestResults).filter(TestResults.test_request_id == assigned_test_request.id).first()
...
def test_schedule_appointment(client: TestClient, test_db: Session, lab_auth_token: str, assigned_test_request: TestRequest):
    ...
    appointment_in_db = test_db.query(Appointment).filter(Appointment.test_request_id == assigned_test_request.id).first()
    assert appointment_in_db is not None
    assert appointment_in_db.status == AppointmentStatus.SCHEDULED

def test_schedule_conflicting_appointment(client: TestClient, lab_auth_token: str, scheduled_appointment: Appointment):
...
def test_cancel_appointment(client: TestClient, test_db: Session, lab_auth_token: str, scheduled_appointment: Appointment):
...
def test_cancel_already_cancelled_appointment(client: TestClient, test_db: Session, lab_auth_token: str, scheduled_appointment: Appointment):
