...
from models import (
    User, Doctor, Patient, MedicalLab, LabStaff, Appointment,
    Symptom, Diagnosis, Consent, TestRequest, Referral, TestResults,
    RoleEnum, SymptomStatus, ConsentPurpose
)
...
class TestUserAndProfileModels:
    ...
    def test_user_creation(self, test_db, patient_user_with_gp, gp_doctor):
        ...
        patient = test_db.query(User).filter_by(username=patient_user_with_gp.username).one()
...
class TestSymptomWorkflowModels:
    ...
    def test_symptom_referral_and_consent(self, test_db, referred_symptom, specialist_doctor):
...
class TestTestingWorkflowModels:
    ...
    def test_test_request_results_relationship(self, test_db, test_result):
        ...
        assert parent_request.test_results[0].summary == "Patient's white blood cell count is elevated."
        
    def test_appointment_uniqueness_constraint(self, test_db, scheduled_appointment, patient_user_with_gp):
        """
    ...
    def test_appointment_relationship_and_status_update(self, test_db, scheduled_appointment):
