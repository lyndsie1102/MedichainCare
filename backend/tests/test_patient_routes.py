...
def test_get_patient_details(client: TestClient, patient_auth_token: str, patient_user):
    ...
    assert response.status_code == 200, response.text  # Adding response.text helps in debugging
    data = response.json()
...
def test_get_patient_details_wrong_role(client: TestClient, doctor_auth_token: str):
    ...
    response = client.get(
        "/patients/patient/me",
        headers={"Authorization": f"Bearer {doctor_auth_token}"} # auth_token is for a doctor
    )
    assert response.status_code == 403
    assert response.json()["detail"] == "User are not authorized to access this endpoint."
...
def test_submit_symptoms_success(client: TestClient, test_db: Session, details_patient_auth_token: str, patient_user_with_gp):
...
def test_get_symptom_details_comprehensive(client: TestClient, patient_auth_token: str, symptom, diagnosis, test_request, appointment, test_result, gp_doctor, lab_staff_user, medical_lab, test_type):
...
def test_get_symptoms_history(client: TestClient, test_db: Session, patient_user, patient_auth_token: str):
    """Test retrieving the symptom history for a patient, including filtering."""
    # Create symptoms for testing
    now = datetime.utcnow()
    s1 = Symptom(patient_id=patient_user.id, symptoms="Old symptom", status="Diagnosed", timestamp=now - timedelta(days=10))
    s2 = Symptom(patient_id=patient_user.id, symptoms="New symptom", status="Pending", timestamp=now - timedelta(days=1))
    s3 = Symptom(patient_id=patient_user.id, symptoms="Another pending", status="Pending", timestamp=now - timedelta(days=2))
    test_db.add_all([s1, s2, s3])
    test_db.commit()

    # Case 1: Get all symptoms
    response = client.get("/patients/symptoms-history/", headers={"Authorization": f"Bearer {patient_auth_token}"})
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 3
    ...
    response = client.get("/patients/symptoms-history/?status=Pending", headers={"Authorization": f"Bearer {patient_auth_token}"})
    assert response.status_code == 200
    ...
    response = client.get(f"/patients/symptoms-history/?start_date={start_date}&end_date={end_date}", headers={"Authorization": f"Bearer {patient_auth_token}"})
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 2
    ...
    response = client.get(f"/patients/symptoms-history/?start_date={end_date}&end_date={start_date}", headers={"Authorization": f"Bearer {patient_auth_token}"})
    assert response.status_code == 400
    ...
    response = client.get(f"/patients/symptoms-history/?start_date={invalid_date}", headers={"Authorization": f"Bearer {patient_auth_token}"})
